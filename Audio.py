...
def get_playlist(day_name):
	"""Get the playlist files for a given day."""
	playlist_dir = './playlist/' + day_name + '/'
	if os.path.isdir(playlist_dir):
		return sorted(os.listdir(playlist_dir)), day_name
	return [], day_name


def bhajans_play(vlc_val=False):						# Using radio sai stream we are playing the bhajans
    global value
    while(value!=0):
        if vlc_available and stream_or_game == 0:
            vlc_player.play()
            if vlc_val:
                vlc_player.stop()
            ...
            playlist, cur_day = get_playlist(cur_day)
